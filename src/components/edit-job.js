...
componentDidMount() {
  axios.get(`http://localhost:5000/jobs/${this.props.match.params.id}`)
    .then(response => {
      const {company, position, description, status, date} = response.data
      this.setState({
      company,
      position,
      description,
      ...
      <div className= "flex flex-col w-full h-12 items-center">
       ...
       <form className="w-full max-w-sm" onSubmit={this.onSubmit}>
         ...
         <div className="">
           ...
           <input type="submit" value="Edit Job Log" className="px-2 py-1 rounded-lg bg-indigo-800 hover:bg-indigo-700 opacity-75  text-yellow-200 hover:text-yellow-400 text-xl font-light uppercase shadow-md hover:shadow-lg" />
       </form>
     </div>
    )
  }
